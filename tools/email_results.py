        ...
        for i, job in enumerate(jobs, 1):
            ...
            if tier:
                ...
                gap_html = (f'<span style="font-size:12px;color:#888;"> · gap: {gap}</span>'
                            if gap else "")
                tier_html = f"""
<div style="background:#f0f7ff;border-left:3px solid #1F3864;padding:10px 14px;
margin:8px 0 12px;border-radius:0 4px 4px 0;">
  <p style="margin:0 0 4px;font-size:12px;font-weight:bold;color:#1F3864;">
    FIT: {badge}{gap_html}
  </p>
  <p style="margin:0;font-size:13px;color:#444;line-height:1.4;">{reason}</p>
            ...
            apply_btn = ""
            if url:
                apply_btn = (f'<a href="{url}" style="background:#1F3864;color:#fff;'
...
if __name__ == "__main__":
    ...
    result = send_digest(fake_jobs, goal="Find top LoadRunner and AI roles")
    print(result["note"])
